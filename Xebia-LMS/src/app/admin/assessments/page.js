...
import { ClipboardList, Plus, Users, Trash2, UserCheck, CheckCircle, PlusCircle, MinusCircle, FileText } from "lucide-react";
              ...
              {form.questions.map((q, qi) => (
                <Card key={q.id} className={`border ${q.type === "written" ? "border-violet-200" : "border-blue-200"}`}>
                  <CardBody className="p-4 space-y-3">
                    ...
                    <Input
                      placeholder={q.type === "written" ? "Written question prompt (e.g. Explain REST principles)" : "MCQ question prompt (e.g. What is dependency injection?)"}
                      value={q.prompt}
                    ...
                    {q.type === "written" && (
                      <div className="bg-violet-50/60 border border-violet-100 rounded-xl p-3">
                        <p className="text-[11px] text-violet-700 font-semibold leading-relaxed">
                          📝 <strong>Written question</strong> — Learners will type a descriptive answer. This must be graded manually by the teacher.
                        </p>
                      </div>
                    )}
                  </CardBody>
