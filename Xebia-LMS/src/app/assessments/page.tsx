...
export default function AssessmentsPage() {
  ...
  const filteredAssessments = assessments.filter((a) => {
    ...
    if (userRole !== "teacher" && userRole !== "admin") {
      ...
      // Check direct email assignment (assignedTo field)
      const assignedByEmail = a.assignedTo && Array.isArray(a.assignedTo) && 
        studentEmail && a.assignedTo.some((e: string) => e.trim().toLowerCase() === studentEmail);

      // Check batch-based assignment
      let assignedByBatch = false;
      if (studentBatch) {
        const batchLower = studentBatch.toLowerCase();
        assignedByBatch = (a.batches && a.batches.some((b: string) => b.trim().toLowerCase() === batchLower)) 
                       || (a.batch && a.batch.trim().toLowerCase() === batchLower);
      }

      // If no batch and no direct assignment → show ALL published assessments as fallback
      const noBatchOrEmail = !studentBatch && !assignedByEmail;
      
      // Block if: learner has a batch/email filter but none match
      if (!noBatchOrEmail && !assignedByBatch && !assignedByEmail) return false;
    } else {
      if (batchFilter !== "All Batches") {
        const selectedBatch = batchFilter.trim().toLowerCase();
        const hasBatch = (a.batches && a.batches.some((b: string) => b.trim().toLowerCase() === selectedBatch)) 
